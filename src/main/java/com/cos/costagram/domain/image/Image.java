...
public class Image {
	...
	@JsonIgnoreProperties({"images"}) //User안에 images는 안불러와도된다. / images무한 참조 방지
	@ManyToOne
	@JoinColumn(name = "userId")
	private User user;
	
	@JsonIgnoreProperties({"image"}) //Tag안에 images는 안불러와도된다.
	@OneToMany(mappedBy = "image")
	private List<Tag> tags;

	@JsonIgnoreProperties({"image"}) //Likes안에 images는 안불러와도된다.
	@OneToMany(mappedBy = "image")
